...
@php $que = DB::table('fmt_lara_ques')->where('id', $message)->first(); @endphp
<div class="fixed z-10 inset-0 overflow-y-auto hidden" id="modalLARA{{$que->id}}">
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      -->
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full relative -mx-8" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <a onclick="closeModalLARA({{$message}})" class="p-2 bg-white w-8 h-8 bg-gray-600 text-white rounded-full absolute right-0 -top-10 -mr-2 -mt-2 z-40" href="javascript:void(0);">x</a>
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <form action="{{route('fmt.lara.update', $que->id)}}" method="post" class="" enctype="multipart/form-data">
                    <div class="fmt_headline">Edit a "Listen the Audio & Choose the words and make a sentence"</div>
                    <div class="my-2">
                        <label class="w-full text-xs" for="">Question</label>
                        <input class="w-full py-1 px-2 border border-gray-500 rounded-lg" type="text" name="question" placeholder="Question" required value="{{$que->question}}">
                    </div>
                    <div class="my-2">
                    ...
                    <div class="my-2">
                        <label class="w-full text-xs" for="">Question audio</label>
                        <input class="w-full py-1 px-2 border border-gray-500 rounded-lg" type="file" accept="audio/*" name="question_media" placeholder="Question">
                    </div>
