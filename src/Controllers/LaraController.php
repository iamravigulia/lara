...
namespace edgewizz\lara\Controllers;
use App\Http\Controllers\Controller;
use App\Models\Media;
use Edgewizz\Edgecontent\Models\ProblemSetQues;
use Edgewizz\Lara\Models\LaraQues;
    ...
    public function imagecsv($question_image, $images){
        foreach($images as $valueImage){
            ...
            if($uploadImage[0] == $question_image){
                // dd($valueImage);
                $media = new Media();
                $valueImage->storeAs('public/question_audio', time() . $valueImage->getClientOriginalName());
                $media->url = 'question_audio/' . time() . $valueImage->getClientOriginalName();
                $media->save();
                return $media->id;
            }
        }
